...
def send_test_sms(sms_to: str):
    project_name = settings.PROJECT_NAME
    subject = f"{project_name} - Test sms"
    with open(Path(settings.EMAIL_TEMPLATES_DIR) / "test_email.html") as f:
        template_str = f.read()
    send_email(
        email_to=email_to,
...
def file_manager(filedir: str, data: UploadFile = File(...)):
    filename, content_type = data.filename, data.content_type
    suffix = content_type.split("/")[1]

    if filedir:
        filepath = f"files/{filedir}/{suffix}/{filename}"

        with open(filepath, "wb+") as file_object:
            shutil.copyfileobj(data.file, file_object)

    return filename, content_type, suffix, filepath
